(() => {
  ...
  function openProvince(pid){
    ...
    $("#provinceImage").alt = `Imagen temporal de ${p.nombre}`;
  ...
  function answer(selected){
    ...
    fb.innerHTML = correct
      ...
      : `<strong>Respuesta incorrecta.</strong> En esta prueba temporal la respuesta configurada es: ${q.opciones[q.correcta]}`;
  ...
  function openAR(pid){
    const url = `ar-prototipo.html?provincia=${encodeURIComponent(pid)}&sesion=${encodeURIComponent(state.sessionId)}`;
    window.open(url, "_blank");
  }

  // Receives capture confirmation from the provisional AR flow.
  function processCapture(payload){
    if(!payload || payload.sessionId !== state.sessionId || !byId[payload.provinceId]) return;
    const s = state.provinces[payload.provinceId];
    if(!s.questionsPassed) return;

    s.badgeCaptured = true;
    beep("win");
    updateMap();
    ...
    $("#mapMessage").textContent = `🏆 Insignia de ${byId[payload.provinceId].nombre} obtenida. La siguiente provincia está disponible.`;
    showScreen("mapScreen");
  }

  if("BroadcastChannel" in window){
    const channel = new BroadcastChannel("geoaventuras-ar");
    channel.addEventListener("message", e => processCapture(e.data));
  }

  window.addEventListener("storage", e => {
    if(e.key === "geoaventuras-ar-capture" && e.newValue){
      try{ processCapture(JSON.parse(e.newValue)); }catch(_){}
    }
  });

  window.addEventListener("focus", () => {
    const raw = localStorage.getItem("geoaventuras-ar-capture");
    if(raw){
      try{ processCapture(JSON.parse(raw)); }catch(_){}
    }
  });
  ...
  function openPassport(){
    ...
    provinces.forEach(p => {
      ...
      item.innerHTML = complete
        ? `<div class="badge-token" style="background:${p.color}">${p.sigla}</div><strong>${p.nombre}</strong><br><small>Insignia obtenida</small>`
  ...
  function downloadPassport(){
    ...
    doc.setFillColor(87,220,254);
    doc.rect(0,0,297,18,"F");

    doc.setFillColor(242,76,23);
    ...
    doc.setFontSize(24);
    doc.text("PASAPORTE GEOAVENTURAS CR",148.5,38,{align:"center"});

    doc.setTextColor(29,44,50);
    doc.setFontSize(16);
    doc.text("¡Misión completada!",148.5,50,{align:"center"});

    doc.setFont("helvetica","normal");
    doc.setFontSize(12);
    doc.text(`Explorador: ${state.player}`,25,70);
    doc.text(`Puntaje obtenido: ${score} / 700 puntos`,25,84);
    doc.text(`Nivel alcanzado: ${level}`,25,98);
    doc.text("Provincias exploradas:",25,116);
    doc.setFontSize(10);
    doc.text("San José · Puntarenas · Heredia · Cartago · Limón · Alajuela · Guanacaste",25,128);
    doc.setFontSize(12);
    doc.text("Insignias obtenidas: 7 de 7",25,145);
    doc.setFontSize(11);
    doc.text("¡Sigue explorando, aprendiendo y descubriendo Costa Rica!",148.5,168,{align:"center"});
    doc.setFont("helvetica","bold");
    doc.text("EXPOTEC 2026 · GeoAventuras CR",148.5,182,{align:"center"});

    doc.save(`Pasaporte-GeoAventuras-${state.player.replace(/\s+/g,"-")}.pdf`);
  }
  ...
  function newPlayer(){
    ...
    runtime = { provinceId:null, questionIndex:0, attemptScore:0, locked:false };
    $("#playerName").value = "";
    localStorage.removeItem("geoaventuras-ar-capture");
    closePassport();
  ...
  document.addEventListener("DOMContentLoaded", () => {
    // Requirement: a new visit always requests a player name.
    state = freshState();
    localStorage.removeItem("geoaventuras-ar-capture");
    showScreen("welcomeScreen");
    ...
    $("#downloadPassportBtn").addEventListener("click", downloadPassport);
