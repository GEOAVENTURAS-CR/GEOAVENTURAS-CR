(() => {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // Enlaces críticos del cuestionario se registran primero.
    on("#nextQuestionBtn", "click", nextQuestion);

    on("#startBtn", "click", startGame);
    on("#playerName", "keydown", e => { if(e.key === "Enter") startGame(); });
    on("#instructionsStartBtn", "click", () => { updateMap(); showScreen("mapScreen"); });
    on("#instructionsBtn", "click", () => showScreen("instructionsScreen"));
    on("#passportBtn", "click", () => { if(state.player) openPassport(); });
    on("#newPlayerBtn", "click", newPlayer);
    on("#finalNewPlayerBtn", "click", newPlayer);
    on("#previewCertificateBtn", "click", openCertificatePreview);
    on("#downloadCertificateBtn", "click", downloadCertificate);
    on("#backFromCertificateBtn", "click", showFinal);

    on("#soundBtn", "click", () => {
      soundEnabled = !soundEnabled;
      const btn = $("#soundBtn");
      if(btn) btn.innerHTML = `${soundEnabled ? "🔊" : "🔇"} <span>Sonido</span>`;
    });
    ...
    on("#activateCameraBtn", "click", ensureCamera);
    on("#showBadgeBtn", "click", showBadge);
    on("#captureBadgeBtn", "click", captureBadge);
    on("#closeArBtn", "click", () => openProvince(currentArProvinceId));
    on("#listenFactBtn", "click", speakCuriousFact);
    on("#continueFromArBtn", "click", () => {
      updateMap();
